...
public class Sequential_RedBlackTree implements RedBlackTree_Interface
{
	...
	{
		if (node.time_executed == -1) 
		{
			return;
		}

		System.out.print(((node.color==RED)?"Color: Red ":"Color: Black ")+"Key: "+node.time_executed+" Parent: "+node.parent.time_executed+"\n");
		printTree(node.left);
	...
	public void insert(Node node) 
	{
		...
		else
		{
			...
			while (true) 
			{
				if (node.time_executed < temp.time_executed) 
				{
					if (temp.left.time_executed == -1) 
					{
						temp.left = node;
						node.parent = temp;
						break;
				...
				else if (node.time_executed >= temp.time_executed)
				{
					if (temp.right.time_executed == -1)
					{
						temp.right = node;
						node.parent = temp;
						break;
	...
	private void fixTree(Node node) 
	{
		...
		while (node.parent.color == RED) 
		{
			...
			if (node.parent == node.parent.parent.left)
			{
				...
				if (uncle.time_executed != -1 && uncle.color == RED) //case1 violation
				{
			...
			else 
			{
				...
				if (uncle.time_executed != -1 && uncle.color == RED)	//case 1 violation
				{
					node.parent.color = BLACK;
					uncle.color = BLACK;
					node.parent.parent.color = RED;
	...
	{
		if (node.parent.time_executed != -1)
		{
			if (node == node.parent.left)
			{
	...
	{ 
		if(target.parent.time_executed == -1)
		{
			root = with;
		}
